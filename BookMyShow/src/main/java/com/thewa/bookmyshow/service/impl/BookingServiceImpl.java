...
import com.thewa.bookmyshow.repository.BookingRepository;
...
public class BookingServiceImpl implements BookingService {
	...
	public Booking hold(User user, Show show, int noOfSeats) throws BookingValidationException {
		...
		if (noOfSeats > Threshold.MAX_SEATS_ALLOWED) {
			throw new BookingValidationException(
					"More than " + Threshold.MAX_SEATS_ALLOWED + " seats can't be booked");
		}
	...
	public Booking book(
		...
		try {
			booking.getShow()
			       .getLock()
			       .lock(); // We take lock on the particular show instance while booking seats
			// to prevent concurrent booking. Since, the booking always happens for the given show, this
			// locking would work.
			
			Booking retrievedBooking =
					this.bookingRepository.getBookingByReferenceNo(booking.getReferenceNumber());
			if (retrievedBooking == null) {
				throw new BookingNotFoundException("Booking is not found in the system");
			}
			
			validateSeatStatus(booking.getShow(),
			                   booking.getSeats(),
			                   new HashSet<>(List.of(SeatStatus.ON_HOLD)));
			this.bookingRepository.putSeatsOnStatus(booking.getShow(), booking.getSeats(),
			                                        SeatStatus.BOOKED);
	...
	public Booking cancel(
		...
		try {
			...
			validateSeatStatus(booking.getShow(),
			                   booking.getSeats(),
			                   new HashSet<>(List.of(SeatStatus.ON_HOLD, SeatStatus.BOOKED)));
			this.bookingRepository.putSeatsOnStatus(booking.getShow(),
			                                        booking.getSeats(),
	...
	private void validateBooking(
			Booking booking, Set<BookingStatus> expectedStatus) throws BookingValidationException {
	...
	private void validateSeatStatus(
			Show show, List<Seat> seats,
			Set<SeatStatus> expectedSeatStatus) throws InvalidSeatStatusException {
