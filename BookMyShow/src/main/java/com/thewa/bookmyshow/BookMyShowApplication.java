package com.thewa.bookmyshow;
import com.thewa.bookmyshow.constant.AudioExperienceType;
import com.thewa.bookmyshow.constant.PaymentStatus;
import com.thewa.bookmyshow.constant.VideoExperienceType;
import com.thewa.bookmyshow.exception.BookingValidationException;
import com.thewa.bookmyshow.model.*;
import com.thewa.bookmyshow.service.BookingService;
import com.thewa.bookmyshow.service.SearchService;
import com.thewa.bookmyshow.service.ShowService;
import com.thewa.bookmyshow.service.impl.BookingServiceImpl;
import com.thewa.bookmyshow.service.impl.SearchServiceImpl;
import com.thewa.bookmyshow.service.impl.ShowServiceImpl;
...
public class BookMyShowApplication {
	private static void test1() throws BookingValidationException {
		List<String> movieNames = Arrays.asList("The Equalizer 3",
		                                        "Barbie",
		...
		List<String> theatreNames = Arrays.asList("PVR Vega City",
		...
		List<Screen> screens =
				Arrays.asList(new Screen(VideoExperienceType.IMAX_2D, AudioExperienceType.DOLBY_DIGITAL),
		...
		for (Theatre theatre : theatres) {
			for (int h = 0; h < theatre.getTotalHalls(); h++) {
				Hall hall = new Hall(String.valueOf(h),
				                     theatre,
				                     screens.get(new Random().nextInt(screens.size())),
				                     100);
		...
		List<Show> searchedShows =
				searchService.search(chosenMovieNames.get(new Random().nextInt(chosenMovieNames.size())),
				                     searchDate);
		...
		System.out.println(
				"Booking status for user " + user1.getName() + " = " + booking1.getStatus().name());
		...
		try {
			...
			System.out.println("Exception for booking " + booking1.getReferenceNumber() + ", error = " +
		...
		System.out.println(
				"Booking status for user " + user2.getName() + " = " + booking2.getStatus().name());
		printSeats(booking2); // Row 1, Seat No - 1, 2, 3, 4 should be on hold since they previously got
		// cancelled
		
		Booking booking3 = bookingService.hold(user2, showToBook, 5);
		System.out.println(
				"Booking status for user " + user2.getName() + " = " + booking3.getStatus().name());
