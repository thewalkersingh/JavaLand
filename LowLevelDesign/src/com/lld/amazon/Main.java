...
public class Main {
	private static void test1(
			PricingService pricingService, OrderService orderService, Customer customer,
		...
		CartService cartService =
				new CartServiceImpl(pricingService, orderService, customer, customerPreferences);
		...
		Order order = cartService.checkout(cart,
		                                   ...
		                                   customer.getProfile().getDefaultDeliveryAddress(),
		                                   deliveryOption);
		...
		for (OrderItemDelivery delivery : orderItemDeliveries) {
			System.out.println(delivery.getItem().getProduct().getTitle() + " " +
			                   delivery.getItem().getChosenVariety().getTitleSuffix() + " has ETA by date:" +
                               " " +
			                   delivery.getDeliveryETA().getETAHumanReadable());
		}
	...
	private static Product getProduct1(PricingService pricingService) throws PricingException {
		String title1 = "Redmi 13C (Starshine Green, 4GB RAM, 128GB Storage) " +
		                "| Powered by 4G MediaTek Helio G85 | 90Hz Display | " + "50MP AI Triple Camera";
		...
		PricingGroup baseGroup =
				pricingService.registerPricingGroup(PricingGroups.BASE, PricingGroupType.ITEM_LEVEL, true);
		PricingGroup taxGroup =
				pricingService.registerPricingGroup(PricingGroups.TAX, PricingGroupType.ITEM_LEVEL, false);
		PricingGroup discountGroup = pricingService.registerPricingGroup(PricingGroups.DISCOUNT,
		                                                                 PricingGroupType.ITEM_LEVEL,
		                                                                 false);
		
		// Base price
		pricingService.registerPricingLineItemWithAmount(baseGroup,
		                                                 PricingLineItems.BASE_AMOUNT,
		                                                 new Money(11000D, CurrencySymbol.INR),
		                                                 null);
		
		// Tax
		pricingService.registerPricingLineItemWithPercentage(taxGroup,
		                                                     PricingLineItems.CGST,
		                                                     2.5D,
		                                                     new Money(CurrencySymbol.INR),
		                                                     baseGroup);
		pricingService.registerPricingLineItemWithPercentage(taxGroup,
		                                                     PricingLineItems.SGST, Double.valueOf(2.5D),
		                                                     new Money(CurrencySymbol.INR),
		                                                     baseGroup);
		
		// Discount
		pricingService.registerPricingLineItemWithPercentage(discountGroup,
		                                                     PricingLineItems.INSTANT_DISCOUNT,
		                                                     Double.valueOf(-10D),
		                                                     new Money(CurrencySymbol.INR),
		                                                     baseGroup);
		pricingService.registerPricingLineItemWithAmount(discountGroup,
		                                                 PricingLineItems.NEW_USER_DISCOUNT,
		                                                 new Money(-50D, CurrencySymbol.INR),
		                                                 baseGroup);
	...
	public static void main(String[] args) throws PricingException {
		CustomerProfile customerProfile =
				new CustomerProfile(CurrencySymbol.INR); // Put addresses in the profile
